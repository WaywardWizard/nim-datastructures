...
type
  BiMap*[T, U] = object
    ## Bidirectional map for unique keys and values
    ab: Table[T, U]
    ba: Table[U, T]

  BiMapSeq*[T,U] = object
    ## Bidirectional map for unique values but shared keys
    ab: Table[T,seq[U]]
...
proc `[]`*[T,U](x: BiMapSeq[T,U], key: T): lent seq[U] = x.ab[key]

proc `[]`*[T,U](x: BiMapSeq[T,U], key: U): lent seq[U] = x.ab[x.ba[key].abkey]

proc `[]=`*[T,U](x: var BiMapSeq[T,U], key: T, val: U) =
  ## put value at key
  if val in x.ba: raise ValueError.newException "Value already stored"
  x.ab.mgetOrPut(key).add val
  x.ba[val]=(x.ab[key].len-1,key)

proc `[]=`*[T,U](x: var BiMapSeq[T,U], val: U, key: T) = x[key] = val

proc contains*[T,U](x: BiMapSeq[T,U], key: T|U): bool =
  when key is T: key in x.ab
  else: key in x.ba

proc find*[T,U](x: BiMapSeq[T,U], val: U): T = x.ba[val].abkey

proc del*[T,U](x: var BiMapSeq[T,U], key: T) =
  ## remove all values
  let vals = x.ab[key]
  x.ab.del key
  for v in vals: x.ba.del v

proc del*[T,U](x: var BiMapSeq[T,U], val: U) =
  let ki = x.ba[val] # abkey, index
  x.ab[ki.abkey].delete ki.index
  if x.ab[ki.abkey].len == 0: x.ab.del ki.abkey
  else:
